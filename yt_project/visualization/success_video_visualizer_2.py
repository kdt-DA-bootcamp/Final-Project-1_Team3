...
def load_success_videos():
    try:
        ...
        for category, group in df.groupby('categoryID'):
            bertopic_keywords_str = group.iloc[0]['bertopic_keywords']
            outliers_str = group.iloc[0]['outliers']

            try:
                bertopic_keywords = json.loads(bertopic_keywords_str)
            except:
                bertopic_keywords = []

            try:
                outliers = json.loads(outliers_str)
            except:
                outliers = []
