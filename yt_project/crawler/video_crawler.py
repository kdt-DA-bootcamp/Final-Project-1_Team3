...
csv_path = os.path.join(script_dir, "..", "data", "카테고리별_키워드_리스트.csv")
...
try:
    for kw in keyword:
        print(f"키워드 '{kw}' 수집 중...")
        video_ids = search_video_ids_by_keyword(kw)
        print(f" - 키워드 '{kw}' 검색된 영상 수: {len(video_ids)}")
        all_video_ids.update(video_ids)
        
        # 각 키워드 처리 후 누적된 데이터로 임시 CSV 저장
        temp_df = pd.DataFrame({"videoID": list(all_video_ids)})
        temp_csv_path = os.path.join("data", "videos_checkpoint.csv")
        temp_df.to_csv(temp_csv_path, index=False, encoding="utf-8-sig", quoting=csv.QUOTE_ALL)
        print(f"중간 결과 저장 완료: {temp_csv_path}")
except KeyboardInterrupt:
    print("사용자가 중단했습니다. 지금까지의 결과를 저장합니다.")
finally:
    print(f"전체 검색된 영상 수: {len(all_video_ids)}")
    if all_video_ids:
        videos_details = get_videos_details(list(all_video_ids))
    ...
    df_videos.to_csv(
        os.path.join("data", "videos_by_keyword.csv"),
        index=False,
        encoding="utf-8-sig",
        quoting=csv.QUOTE_ALL
    )
    print("영상 데이터 저장 완료: data/videos_by_keyword.csv")
    ...
    df_channels.to_csv(
        os.path.join("data", "channels_by_keyword.csv"),
        index=False,
        encoding="utf-8-sig",
        quoting=csv.QUOTE_ALL
    )
    print("채널 데이터 저장 완료: data/channels_by_keyword.csv")